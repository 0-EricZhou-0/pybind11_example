...
# Description:
#   Compiles protobuf files into C++ and/or Python sources.
# Arguments:
#   [Positional] NAME: Name of the compilation target.
#   SOURCE_DIR: Directory containing the .proto source files.
#   [Optional] CXX_DEST_DIR: Directory to output generated C++ sources.
#   [Optional] PY_DEST_DIR: Directory to output generated Python sources.
#   [Optional] [Return] GEN_SOURCES: Variable to store the list of generated source files.
#   [MultiVal] SOURCES: List of .proto source files to compile.
function(proto_compile name)
    ...
    if(DEFINED ARG_GEN_SOURCES AND NOT ${ARG_GEN_SOURCES} STREQUAL "")
...
# Function cxx_setup_target
# Description:
#   Sets up a C/C++ target with sources, includes, dependencies, compile options, and link options.
#   Also setup a cmake target which responsible for building the target.
# Arguments:
#   [Positional] NAME: Name of the target to set up.
#   [MultiVal] SOURCES: Will be ignored, present here to remain compatible with cxx_add_* functions.
#   [MultiVal] INCLUDES: List of include directories for the target.
#   [MultiVal] DEPENDS: List of dependencies (libraries) for the target.
#   [MultiVal] COPTIONS: List of compile options for the target.
#   [MultiVal] LOPTIONS: List of link options for the target.
# Modified Targets:
#   ${NAME}:
#       Target will be setup with the specified includes, dependencies, compile options, and link
#       options.
function(cxx_setup_target name)
    cmake_parse_arguments(ARG "" "NAME" "SOURCES;INCLUDES;DEPENDS;COPTIONS;LOPTIONS" ${ARGN})
    ...
    if(EXPORT_TARGET_CONFIG)
        ...
        string(REPLACE ";" ${REPLACE_STR} TARGET_SOURCES "${ARG_SOURCES}")
        string(REPLACE ";" ${REPLACE_STR} TARGET_INCLUDES "${ARG_INCLUDES}")
        string(REPLACE ";" ${REPLACE_STR} TARGET_DEPENDS "${ARG_DEPENDS}")
        # list all coptions and loptions in one line
        string(REPLACE ";" " " TARGET_COPTIONS "${ARG_COPTIONS}")
        string(REPLACE ";" " " TARGET_LOPTIONS "${ARG_LOPTIONS}")

        string(CONCAT TARGET_CONFIG_PRINTOUT
            "Cmake at pwd: ${CMAKE_CURRENT_SOURCE_DIR}\n"
            "  Target:   ${name}\n"
            "  Sources:  ${TARGET_SOURCES}\n"
            "  Includes: ${TARGET_INCLUDES}\n"
            "  Depends:  ${TARGET_DEPENDS}\n"
            "  Compile Options: ${TARGET_COPTIONS}\n"
            "  Link Options: ${TARGET_LOPTIONS}\n"
        )
        message(STATUS ${TARGET_CONFIG_PRINTOUT})
    endif()
endfunction()

# Function cxx_add_executable
# Description:
#   Adds an executable target with specified properties. Argument list same as cxx_setup_target.
# Arguments:
#   [Positional] NAME: Name of the target to set up.
#   [Optional] [Return] TARGET: Variable to store the target name.
#   [MultiVal] SOURCES: List of source files for the target.
#   [MultiVal] INCLUDES: List of include directories for the target.
#   [MultiVal] DEPENDS: List of dependencies (libraries) for the target.
#   [MultiVal] COPTIONS: List of compile options for the target.
#   [MultiVal] LOPTIONS: List of link options for the target.
# Generated Targets:
#   ${NAME}: Executable build target.
...
# Function cxx_add_static_library
# Description:
#   Adds an static library target with specified properties. Argument list same as cxx_setup_target.
# Arguments:
#   [Positional] NAME: Name of the target to set up.
#   [Optional] [Return] TARGET: Variable to store the target name.
#   [MultiVal] SOURCES: List of source files for the target.
...
#   [MultiVal] LOPTIONS: List of link options for the target.
# Generated Targets:
#   ${NAME}: Static library build target.
...
# Function cxx_add_dynamic_library
# Description:
#   Adds an dynamic library target with specified properties. Argument list same as cxx_setup_target.
# Arguments:
#   [Positional] NAME: Name of the target to set up.
#   [Optional] [Return] TARGET: Variable to store the target name.
#   [MultiVal] SOURCES: List of source files for the target.
...
#   [MultiVal] LOPTIONS: List of link options for the target.
# Generated Targets:
#   ${NAME}: Dynamic library build target.
...
# Function cxx_add_module
# Description:
#   Adds an module target with specified properties. Argument list same as cxx_setup_target.
# Arguments:
#   [Positional] NAME: Name of the target to set up.
#   [Optional] [Return] TARGET: Variable to store the target name.
#   [MultiVal] SOURCES: List of source files for the target.
